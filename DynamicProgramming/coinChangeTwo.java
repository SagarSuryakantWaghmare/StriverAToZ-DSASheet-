...
public class coinChangeTwo {
    public static int coinChange(int[] coins, int amount, int idx) {
        if (amount == 0) return 1;        // base case: valid combination
        if (idx < 0 || amount < 0) return 0; // base case: no valid way
    ...
    public static void main(String[] args) {
        ...
        int amount = 5;
        int n=coins.length;
        // System.out.println("Total Number of ways to make amount: " + coinChange(coins, amount, coins.length - 1));
        // int dp[][]=new int[n][amount+1];
        // for(int row[]:dp){
        //     Arrays.fill(row, -1);
        // }
        // System.out.println("Total Number of ways to make amount: " + coinChangeMem(coins, amount, coins.length - 1,dp));
